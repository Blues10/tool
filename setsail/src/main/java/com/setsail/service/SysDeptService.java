...
import com.setsail.dao.SysDeptMapper;
...
import com.setsail.util.LevelUtil;
...
public class SysDeptService {
    ...
    public void update(DeptParam param) {
        ...
        Preconditions.checkNotNull(param, "待更新的部门不存在");
        if (checkExist(param.getParentId(), param.getName(), param.getId())) {
            throw new ParamException("同一层级下存在相同名称的部门");
        }
        ...
        after.setLevel(LevelUtil.calculateLevel(getLevel(param.getParentId()), param.getParentId()));
        after.setOperator("system_update"); //TODO
        after.setOperateIp("127.0.0.1"); //TODO
        after.setOperateTime(new Date());
        updateWithChind(before,after);
    }

    @Transactional
    private void updateWithChind(SysDept before, SysDept after) {
